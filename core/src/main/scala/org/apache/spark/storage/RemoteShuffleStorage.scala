...
private[spark] object RemoteShuffleStorage extends Logging {
  ...
  lazy val remoteStoragePath = new Path(SparkEnv.get.conf.get("spark.shuffle.remote.storage.path"))
  ...
  def registerBlockManagerifNeeded(master: BlockManagerMaster, conf: SparkConf,
    ...
    if (conf.getOption("spark.shuffle.remote.storage.path").isDefined) {
  ...
  def cleanUp(conf: SparkConf, hadoopConf: Configuration): Unit = {
    if (conf.contains("spark.app.id") &&
        conf.getOption("spark.shuffle.remote.storage.path").isDefined) {
      val shuffleRemotePath =
        new Path(conf.get("spark.shuffle.remote.storage.path"), conf.getAppId)
