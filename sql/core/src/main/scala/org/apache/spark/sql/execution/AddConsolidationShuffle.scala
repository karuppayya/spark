...
import org.apache.spark.sql.execution.exchange.{ConsolidationShuffleExchangeExec, ENSURE_REQUIREMENTS, ShuffleExchangeExec}
import org.apache.spark.sql.internal.SQLConf

object AddConsolidationShuffle extends Rule[SparkPlan] {

  def apply(plan: SparkPlan): SparkPlan = {
    if (!SQLConf.get.getConfString(
        "spark.sql.shuffle.consolidation.enabled", "false").toBoolean) {
      return plan
    }
    plan transformUp  {
      case plan@ShuffleExchangeExec(part, _, ENSURE_REQUIREMENTS, _)
        if !SQLConf.get.adaptiveExecutionEnabled =>
        ConsolidationShuffleExchangeExec(plan, part)
      case parent if SQLConf.get.adaptiveExecutionEnabled &&
          parent.children.exists(_.isInstanceOf[ShuffleQueryStageExec]) =>

        parent.mapChildren {
          case stage: ShuffleQueryStageExec
            if !stage.shuffle.isInstanceOf[ConsolidationShuffleExchangeExec] &&
              stage.isMaterialized =>
            // Threshold check
            val size = stage.getRuntimeStatistics.sizeInBytes
            val consolidationThreshold = org.apache.spark.network.util.JavaUtils
              .byteStringAsBytes(SQLConf.get.getConfString(
                "spark.sql.shuffle.consolidation.size.threshold", "100MB"))
            if (size > consolidationThreshold) {
              val consolidation = ConsolidationShuffleExchangeExec(
                stage, stage.outputPartitioning)
  ...
  private def setLogicalLinkForConsolidation(
      consolidation: ConsolidationShuffleExchangeExec,
      parent: SparkPlan,
