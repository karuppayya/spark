...
class RedshiftDatasourceV2 extends FileDataSourceV2 with DataSourceRegister with Logging  {
  ...
  val params: MergedParameters = null
  ...
  override def getTable(options: CaseInsensitiveStringMap, schema: StructType): Table = {
    if (table.isEmpty) {
      val paths = getPaths(options)
      val tableName = getTableName(paths)
      table = Some(new RedshiftTable(tableName,
        sparkSession, options, paths, None, fallbackFileFormat))
    }
    table.get
  }
  ...
  def preBuild(tableNameOrSubquery: String): Unit = {
    val params: MergedParameters = null;
    ...
    if (table.get.schema().isEmpty) {
      ...
      val unloadSql = buildUnloadStmt(table.get.schema().map(_.name).toArray[String],
      ...
      val prunedSchema = pruneSchema(table.get.schema(), table.get.schema().map(_.name).toArray)
