...
package org.apache.spark.sql.execution.datasources.redshift

import org.scalatest.Matchers

import org.apache.spark.SparkFunSuite
import org.apache.spark.sql.execution.datasources.v2.redshift.{Parameters, TableName}
...
class ParametersSuite extends SparkFunSuite with Matchers {
  ...
  test("validate unload format option ") {
    val unloadFormatStr = "unloodformat"
    val params = Map(
      "forward_spark_s3_credentials" -> "true",
      "tempdir" -> "s3://foo/bar",
      "dbtable" -> "test_schema.test_table",
    ...
    Parameters.mergeParameters(params + (unloadFormatStr -> "csv"))
    Parameters.mergeParameters(params + (unloadFormatStr -> "parquet"))
    ...
    intercept[IllegalArgumentException] {
      Parameters.mergeParameters(params + (unloadFormatStr -> "invalid-temp-format"))
    }
  }
